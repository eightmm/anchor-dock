...
GOOD_COMPATIBILITY: dict[str, set[str]] = {
    ...
    "fluoroacetamide": {"CYS"}, "chlorofluoroacetamide": {"CYS"},
...
class AnchorPoint:
    ...
    def reactive_target(self) -> np.ndarray:
        return self.coord + self.bond_vector * self.bond_length
...
def detect_warheads(mol: Chem.Mol) -> list[WarheadHit]:
    ...
    candidates.sort(key=lambda hit: -len(hit.matched_atoms))
...
def create_covalent_coordmap(
    ...
    """Return exact support, nucleophile and electrophile coordinates."""
    return {
        support_atom_idx: Point3D(*map(float, anchor.support_coord)),
        nucleophile_atom_idx: Point3D(*map(float, anchor.coord)),
        reactive_atom_idx: Point3D(*map(float, anchor.reactive_target)),
    }
